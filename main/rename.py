...
async def rename_file(bot, msg):
    ...
    filename_without_extension = os.path.splitext(new_name)[0]  # Get filename without extension
    max_length = 60 - len(extension)
    
    if len(filename_without_extension) > max_length:
        filename_without_extension = filename_without_extension[:max_length]  # Truncate to fit within 60 characters
    
    new_name_with_extension = filename_without_extension + extension  # Final filename with extension
